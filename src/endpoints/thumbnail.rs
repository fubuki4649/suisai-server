...
pub async fn get_thumbnail(hash: &str) -> Result<NamedFile, Status> {
    let mut conn = unwrap_or_return!(DB_POOL.get());
    let photo = unwrap_or_return!(check_hash(&mut conn, hash));

    match photo {
        Some(photo) => Ok(unwrap_or_return!(NamedFile::open(photo.thumbnail_path).await)),
